<template>
  <section class="h-full flex flex-col md:flex-row justify-center items-center">
    ...
    <div class="flex-11/12 md:flex-1/2 flex items-center justify-center">
      <div class="w-full grid grid-cols-3 lg:grid-cols-4 gap-4 px-5">
        <UPageCard
          v-for="(data, i) in dataImage"
          :key="i"
          class="flex justify-center items-center h-16 md:h-32"
          spotlight
        >
          <div class="flex justify-center items-center w-full h-12 md:h-20" data-aos="fade-up"
...
<script setup>
  const dataImage = ref(['html','css','js','php','go','laravel','node','nuxt','vue','tailwind','pgsql','mongo'])
</script>
